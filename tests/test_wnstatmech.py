...
def test_wide_degenerate_batch_matches_scalar_quantities():
    electron = ws.fermion.create_electron()
    temperatures = np.geomspace(1.0e7, 1.0e10, 17)
    alphas = np.linspace(0.25, 16.25, 17)
...
def test_direct_chemical_potential_skips_numerical_solve(monkeypatch):
    electron = ws.fermion.create_electron()
    electron.update_chemical_potential_function(lambda T, n: -12.5)

    def numerical_solve_was_called(*args):
        raise AssertionError("Numerical chemical-potential solve was called")

    monkeypatch.setattr(
        electron,
        "_compute_chemical_potential_numerical",
        numerical_solve_was_called,
    )

    assert electron.compute_chemical_potential(1.0e8, 1.0e20) == -12.5


def test_direct_chemical_potential_fallback_matches_numerical_result():
    temperature = 1.0e8
    alpha = -5.0
    reference = ws.fermion.create_electron()
    number_density = reference.compute_quantity(
        "number density", temperature, alpha
    )
    expected = reference.compute_chemical_potential(
        temperature, number_density
    )

    electron = ws.fermion.create_electron()
    electron.update_chemical_potential_function(lambda T, n: None)

    result = electron.compute_chemical_potential(temperature, number_density)

    assert np.isclose(result, expected, atol=1.0e-8)


def test_direct_chemical_potential_can_be_replaced_and_disabled():
    electron = ws.fermion.create_electron()
    temperature = 1.0e8
    alpha = -5.0
    ...
    electron.update_chemical_potential_function(lambda T, n: -10.0)
    assert (
        electron.compute_chemical_potential(temperature, number_density)
        == -10.0
    )

    electron.update_chemical_potential_function(lambda T, n: -11.0)
    assert (
        electron.compute_chemical_potential(temperature, number_density)
        == -11.0
    )

    electron.update_chemical_potential_function(None)
    assert np.isclose(
        electron.compute_chemical_potential(temperature, number_density),
        alpha,
        atol=1.0e-8,
...
def test_vectorized_direct_chemical_potential_mixes_direct_and_fallback():
    electron = ws.fermion.create_electron()
    temperatures = np.array([1.0e7, 1.0e8, 1.0e9])
    alphas = np.array([-5.0, -1.0, 2.0])
    number_densities = electron.compute_quantity(
        "number density", temperatures, alphas
    )
    callback_temperatures = []

    def direct_function(temperature, number_density):
        callback_temperatures.append(temperature)
        if temperature == temperatures[0]:
            return alphas[0]
        return None

    electron.update_chemical_potential_function(direct_function)
    result = electron.compute_chemical_potential(
        temperatures, number_densities
    )

    assert np.allclose(result, alphas, atol=1.0e-5)
    assert callback_temperatures == list(temperatures)


def test_direct_chemical_potential_callback_exceptions_propagate():
    electron = ws.fermion.create_electron()

    def direct_function(temperature, number_density):
        raise ValueError("invalid approximation state")

    electron.update_chemical_potential_function(direct_function)

    with pytest.raises(ValueError, match="invalid approximation state"):
        electron.compute_chemical_potential(1.0e8, 1.0e20)
...
def test_invalid_cache_sizes_raise():
    with pytest.raises(ValueError, match="Invalid cache size"):
        ws.fermion.create_electron(cache_size=-1)

    with pytest.raises(ValueError, match="Invalid cache size"):
        ws.fermion.create_electron(cache_size=1.5)


def test_quantity_cache_is_bounded_and_uses_lru_order():
    electron = ws.fermion.create_electron(cache_size=2)
    temperature = 1.0e8

    electron.compute_quantity("pressure", temperature, -5.0)
    electron.compute_quantity("pressure", temperature, -4.0)
    electron.compute_quantity("pressure", temperature, -5.0)
    electron.compute_quantity("pressure", temperature, -3.0)

    cached_alphas = [key[2] for key in electron._quantity_cache]
    assert cached_alphas == [-5.0, -3.0]


def test_zero_cache_size_disables_exact_result_caches():
    electron = ws.fermion.create_electron(cache_size=0)
    temperature = 1.0e8
    alpha = -5.0
    number_density = electron.compute_quantity(
        "number density", temperature, alpha
    ...
    electron.compute_quantity("pressure", temperature, alpha)
    electron.compute_chemical_potential(temperature, number_density)

    assert not electron._quantity_cache
    assert not electron._chemical_potential_cache


def test_clear_cache_removes_exact_results_and_warm_start():
    electron = ws.fermion.create_electron()
    temperature = 1.0e8
    alpha = -5.0
    number_density = electron.compute_quantity(
    ...
    assert electron._quantity_cache
    assert electron._chemical_potential_cache
    assert electron._chemical_potential_seed

    electron.clear_cache()

    assert not electron._quantity_cache
    assert not electron._chemical_potential_cache
    assert not electron._chemical_potential_seed
